  ...
  moduleConfig = ( {
    ...
    const dataUrlCondition = {
      ...
      parserJavascriptConfig = {
        ...
        // v5.108.0+开始启用该选项。其值类型为：string[]。不支持空数组，至少数组内有1个成员。
        // 其数组中的成员表示：模块中在调用时应视为无副作用的顶级函数名称。将列出的顶级函数名称标记为基于纯函数的树摇动对象。
        // 将列出的顶级函数名称标记为“无副作用”，可以被树抖安全删除！！！
        // 以便对这些函数的调用（其结果未被使用）可以被进行树摇操作。
        // 这是 /*#__NO_SIDE_EFFECTS__*/ 注释的显式配置对应方式。
        // 无需在源代码中添加注释，而是直接列出函数名称。
        // 使用“default”可将目标指向默认导出的函数。
        // 最好按规则配合测试来应用，这样名称只会在预期模块中被标记为无副作用。
        ...( () => {
