...
const isProduction = ( argv => {
  ...
  maxImgSize = 10 * 1024,
  /**
   * 1、v5.108.0+开始启用该选项。其值类型为：string[]。不支持空数组，至少数组内有1个成员。<br />
   * 2、其数组中的成员表示：模块中在调用时应视为无副作用的顶级函数名称。将列出的顶级函数名称标记为基于纯函数的树摇动对象。<br />
   * 3、将列出的顶级函数名称标记为“无副作用”，可以被树抖安全删除！！！<br />
   * 4、以便对这些函数的调用（其结果未被使用）可以被进行树摇操作。<br />
   * 5、无需在源代码中添加注释，而是直接列出函数名称。<br />
   * 6、使用“default”可将目标指向默认导出的函数。<br />
   * 7、最好按规则配合测试来应用，这样名称只会在预期模块中被标记为无副作用。<br />
   * 8、这是 /星号#__NO_SIDE_EFFECTS__星号/ 注释的显式配置对应方式。<br />
   * 9、截至2026.06.30，有5种标记“无副作用”的方式！以便可以被树抖安全删除：<br />
   *   1)项目根目录下“package.json”中的顶级配置项“sideEffects”，其值设置成false表示项目整体都是无副作用的！整个项目可以被树抖安全处理！<br />
   *     其值设置成true表示项目整体都是有副作用的！具体的树抖，交由代码、webpack配置具体处理！<br />
   *     其值设置成[ "星号星号/星号.css", ... ]，表示匹配数组里的各个正则匹配模式的文件，都是有副作用的，不会被树抖处理！<br />
   *   2)/星号 #__PURE__ 星号/用于标记函数调用，该注解会将语句标记为无副作用。可以被树抖安全删除：<br />
   *   ```js
   *   const Button$1 = /星号 #__PURE__ 星号/ withAppProvider()(Button);
   *   ```<br />
   *   3)/星号 #__NO_SIDE_EFFECTS__ 星号/用于标记函数定义、描述！用于将函数声明标记为纯函数。<br />
   *     当带有此注解的函数的返回值未被使用时，即使该函数体无法通过静态分析判定为纯函数，对其的调用仍可从打包结果中移除：<br />
   *     ```js
   *     /星号 #__NO_SIDE_EFFECTS__ 星号/
   *     export function createLogger(prefix) {
   *       return (msg) => console.log(`[${prefix}] ${msg}`);
   *     }
   *     ```<br />
   *   4)module.parser.javascript.pureFunctions（就是本常量设置的）将名称标记为无副作用。可以被树抖安全删除！<br />
   *   5)Module.Rule.sideEffects，其值类型只支持布尔值类型，标记模块有无副作用。<br />
   *     false表示无副作用！！！可以被树抖安全删除！！！<br />
   *     true表示有副作用！！！不会被树抖处理！！！<br />
   *     例子：<br />
   *     ```js
   *     module: {
   *       rules: [
...
const browserslist = [
    ...
    // PC端各主流浏览器的最新版本，至20260701。Start
    'Chrome >= 150',
    ...
    'Safari >= 26',
    'Opera >= 133',
    // PC端各主流浏览器的最新版本，至20260701。End

    // 移动端各主流浏览器的最新版本，至20260701。Start
    'ChromeAndroid >= 150',
    // 从Android 4.4后Android WebView直接跟Chrome同步。
    'Android >= 150',
    'FirefoxAndroid >= 152',
    'iOS >= 26',
    // 移动端各主流浏览器的最新版本，至20260701。End
  ],
  /**
...
\n` );
  ...
  moduleConfig = ( {
    ...
    const babelParserPlugins = [
      ...
      babelPlugins = [
        ...
        // 用于生产模式下，删除代码中的“console”、“debugger”。
        ...( () => {
          if( env_platform === 'dev_server' ){
            return [];
          }
        ...
        ...( isEnable => {
          ...
          const configs = {
            ...
            sharpMinify: new ImageMinimizerPlugin( {
              ...
              /**
               * 目前支持从其他格式生成：jpeg（别名：jpe、jpg）、png、webp、gif、jp2、tiff（别名：tif）、avif、heif（别名：heic）、jxl、raw格式的图片。直到20260614，sharp v0.35.1、vips-dev-x64-all-8.18.3，转换heic、heif还是会报错。<br />
  ...
  performanceConfig = {
    ...
    assetFilter( assetFilename ){
      const arr1 = Array.from( new Set( JSON.parse( '["cson","css","csv","tsv","ejs","eot","otf","fon","font","ttf","ttc","woff","woff2","graphql","graphqls","gql","handlebars","hbs","htm","html","xhtml","apng","arw","avif","bmp","bpg","cr2","cur","dcx","dng","flif","gif","heic","heif","icns","ico","j2c","j2k","jbig2","jng","jp2","jpe","jfif","pjpeg","pjp","jpeg","jpg","jpm","jpx","jxl","jxr","ktx","mj2","nef","orf","pam","pbm","pcx","pgm","png","pnm","ppm","psd","raf","raw","rgbe","rw2","svg","svgz","tga","tif","tiff","wbmp","webp","wp2","xbm","xpm","js","cjs","mjs","jsx","json5","less","toml","txt","json","webmanifest","markdown","md","m4a","kar","ape","wav","wave","flac","wma","cda","aiff","au","mpeg","mpeg-1","mpeg-2","mpeg-layer3","mpeg-4","opus","mp3","mp2","mp1","mid","midi","ra","rm","rmx","vqf","amr","aac","vorbis","mustache","pcss","postcss","pug","jade","sass","scss","styl","stylus","sss","ts","mts","cts","tsx","wmv","asf","asx","rmvb","mp4","3gp","mov","m4v","avi","dat","mkv","flv","vob","mod","mng","mpg","3gpp","ogg","webm","vtt","vue","wasm","xml","yaml","yml"]' ) ) );
