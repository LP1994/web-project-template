...
( function ( UPNG, pako ){
  ...
  UPNG.encode.compress = function ( bufs, w, h, ps, forGIF, forbidPlte ){
    ...
    var brute = gotAlpha
                ? forGIF
                : false;		// brute : frames can only be copied, not "blended"
    ...
    for(
      ...
      var nx = 0,
        ...
        blend = 0;
      if( j != 0 && !brute ){
        var tlim = ( forGIF || j == 1 || frms[ frms.length - 2 ].dispose == 2 )
                   ? 1
                   : 2,
          tstp = 0,
          tarea = 1e9;
