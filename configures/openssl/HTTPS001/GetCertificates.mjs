 ...
 * FileDirPath: openssl/HTTPS001/GetCertificates.mjs
 * Author: 12278
...
function GetCertificates(){
  const keyFile = readFileSync( join( __dirname, './HTTPS001Server.key' ), 'utf8' ),
    certFile = readFileSync( join( __dirname, './HTTPS001Server.crt' ), 'utf8' );
  ...
  return new Promise( ( resolve, reject ) => {
    pem.createCSR( {
      ...
      organization: 'HTTPS001',
      // Common Name (e.g. server FQDN or YOUR name) []
      commonName: '192.168.10.101',
...
writeFileSync( join( __dirname, './HTTPS001Cert.pem' ), myCertificates.cert, {
  flag: 'w+',
  encoding: 'utf8',
} );
writeFileSync( join( __dirname, './HTTPS001Key.pem' ), myCertificates.key, {
  flag: 'w+',
  encoding: 'utf8',
} );
