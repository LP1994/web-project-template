/**
 * Project: sn-data-center-platform-micro-front-web<br />
 * Author：12278<br />
 * Email：1227839175@qq.com<br />
 * CreateDate：2020-06-01 00:00:00<br />
 * IDE: WebStorm
 */

    // process.cwd()输出G:\\WebStormWS\\sn-data-center-platform-micro-front-web
let isPro = ( () => process.argv[ process.argv.findIndex( ( c, i, a ) => c === '--mode' ) + 1 ] === 'production' )(),
    fs = require( 'fs' ),
    path = require( 'path' ),
    os = require( 'os' ),
    // 返回的数值：系统内存总量的一半，且舍去小数。8192、Math.trunc( os.totalmem() / 2147483648 )
    maxOldSpaceSize4Recommend = 8192,
    toml = require( 'toml' ),
    yaml = require( 'yamljs' ),
    ...
    jsWorkerPool = {
        ...
        // 其他的node.js参数
        workerNodeArgs: [
            `--max-old-space-size=${ maxOldSpaceSize4Recommend }`,
        ],
        ...
        poolTimeout: isPro
                     ? 10000
                     : Infinity,
        // 池分配给工人的作业数默认为200个，分配效率较低但更合理的的分配
        poolParallelJobs: osLen * 20,
...
threadLoader.warmup( vueWorkerPool, [
    'vue-loader',
] );

// 直到20200908，只支持的列表如下！！！
let compilerOptions4lib_arrC = [
        'dom',
        'dom.iterable',
        'es2015',
        'es2015.collection',
    ...
    output_fun = ( {
        ...
        // 'web'：'jsonp'、'WebWorker'：'importScripts'、'sync node.js'：'require'、'async node.js'：'async-node'
        chunkLoading: 'jsonp',
        ...
        /*
         ...
         也可以指定绝对路径：'http://localhost:8081/sn-data-center-platform-micro-front-web/dist/production/'，一般用于正式生产环境
         */
        publicPath: '../',
        environment: {
            'arrowFunction': true,
        ...
        // library选项的有效值：[non-empty string, ...] | non-empty string | object { amd?, commonjs?, root? } | object { type, auxiliaryComment?, export?, name?, umdNamedDefine? }
        // library,
    } ),
    ...
    defineObj_fun = isPro => {
        ...
        return {
            ...
            devURL8081A: isPro
                         ? '"/"'
                         : '"/devURL8081A/"',
            // websocket，开发服务器(连公司有线局域网，端口：8081) ws://192.168.1.168:8081/subscriptions
            ws4DevURL8081A: isPro
                            ? `${ str1 }"/"`
                            : `${ str1 }"/ws4DevURL8081A/"`,

            // websocket，开发服务器(连公司有线局域网，端口：18081)上暂时先用的临时的WebSocket接口地址(打正式包的时候会用到，注意向后端询问其变动情况) ws://192.168.1.168:18081/ibms/subscriptions
            ws4IBMS4DevURL18081A: isPro
                                  ? `( location.protocol === "http:" ? "ws:" : "wss:" ) + "//" + location.hostname + ":18081" + "/ibms/"`
                                  : `${ str1 }"/ws4IBMS4DevURL18081A/"`,

            // 内网穿透(目前指向公司刘家敏的台式机9088端口，它会转发给公司的开发服务器192.168.1.168:8081) http://sn2020a.nat300.top/graphql
            devURL4Natapp: isPro
                           ? '"/"'
                           : '"/devURL4Natapp/"',
            // websocket，内网穿透(目前指向公司刘家敏的台式机9088端口，它会转发给公司的开发服务器192.168.1.168:8081) ws://sn2020a.nat300.top/subscriptions
            ws4DevURL4Natapp: isPro
                              ? `${ str1 }"/"`
                              : `${ str1 }"/ws4DevURL4Natapp/"`,

            // 测试服务器(连公司有线局域网，端口：8080) http://192.168.1.168:8080/graphql
            testURL: isPro
                     ? '"/"'
                     : '"/testURL/"',
            // websocket，测试服务器(连公司有线局域网，端口：8080) ws://192.168.1.168:8080/subscriptions
            ws4TestURL: isPro
                        ? `${ str1 }"/"`
                        : `${ str1 }"/ws4TestURL/"`,

            // 邓龙光(连公司有线局域网，端口：8099) http://192.168.1.183:8099/graphql
            devURL4DLG8099A: isPro
                             ? '"/"'
                             : '"/devURL4DLG8099A/"',
            // 邓龙光(连刘家敏电脑的无线网络，端口：8099) http://192.168.137.236:8099/graphql
            devURL4DLG8099B: isPro
                             ? '"/"'
                             : '"/devURL4DLG8099B/"',
            // websocket，邓龙光(连公司有线局域网，端口：8099) ws://192.168.1.183:8099/subscriptions
            ws4DevURL4DLG8099A: isPro
                                ? `${ str1 }"/"`
                                : `${ str1 }"/ws4DevURL4DLG8099A/"`,
            // websocket，邓龙光(连刘家敏电脑的无线网络，端口：8099) ws://192.168.137.236:8099/subscriptions
            ws4DevURL4DLG8099B: isPro
                                ? `${ str1 }"/"`
                                : `${ str1 }"/ws4DevURL4DLG8099B/"`,

            // 许锦滨(连公司有线局域网，端口：8099) http://192.168.1.119:8099/graphql
            devURL4XJB8099A: isPro
                             ? '"/"'
                             : '"/devURL4XJB8099A/"',
            // 许锦滨(连刘家敏电脑的无线网络，端口：8099) http://192.168.137.73:8099/graphql
            devURL4XJB8099B: isPro
                             ? '"/"'
                             : '"/devURL4XJB8099B/"',
            // websocket，许锦滨(连公司有线局域网，端口：8099) ws://192.168.1.119:8099/subscriptions
            ws4DevURL4XJB8099A: isPro
                                ? `${ str1 }"/"`
                                : `${ str1 }"/ws4DevURL4XJB8099A/"`,
            // websocket，许锦滨(连刘家敏电脑的无线网络，端口：8099) ws://192.168.137.73:8099/subscriptions
            ws4DevURL4XJB8099B: isPro
                                ? `${ str1 }"/"`
                                : `${ str1 }"/ws4DevURL4XJB8099B/"`,

            // 李东明(连刘家敏电脑的无线网络，端口：8081) http://192.168.137.182:8081/graphql
            devURL4LDM8081A: isPro
                             ? '"/"'
                             : '"/devURL4LDM8081A/"',
            // websocket，李东明(连刘家敏电脑的无线网络，端口：8081) ws://192.168.137.182:8081/subscriptions
            ...
            devURL4DHT8081A: isPro
                             ? '"/"'
                             : '"/devURL4DHT8081A/"',
            // websocket，戴海涛(连公司有线局域网，端口：8081) ws://192.168.1.147:8081/subscriptions
            ws4DevURL4DHT8081A: isPro
                                ? `${ str1 }"/"`
                                : `${ str1 }"/ws4DevURL4DHT8081A/"`,

            // 转发，websocket，开发服务器(连公司有线局域网，端口：18081)上暂时先用的临时的WebSocket接口地址(打正式包的时候会用到，注意向后端询问其变动情况) ws://192.168.1.168:18081/ibms/subscriptions
            Natapp4WS4IBMS4DevURL18081A: isPro
                                         ? `( location.protocol === "http:" ? "ws:" : "wss:" ) + "//" + location.hostname + ":18081" + "/ibms/"`
                                         : `${ str1 }"/Natapp4WS4IBMS4DevURL18081A/"`,

            // 转发，测试服务器(连公司有线局域网，端口：8080) http://192.168.1.168:8080/graphql
            Natapp4TestURL: isPro
                            ? '"/"'
                            : '"/Natapp4TestURL/"',
            // 转发，websocket，测试服务器(连公司有线局域网，端口：8080) ws://192.168.1.168:8080/subscriptions
            Natapp4WS4TestURL: isPro
                               ? `${ str1 }"/"`
                               : `${ str1 }"/Natapp4WS4TestURL/"`,

            // 转发，邓龙光(连公司有线局域网，端口：8099) http://192.168.1.183:8099/graphql
            Natapp4DevURL4DLG8099A: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4DLG8099A/"',
            // 转发，邓龙光(连刘家敏电脑的无线网络，端口：8099) http://192.168.137.236:8099/graphql
            Natapp4DevURL4DLG8099B: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4DLG8099B/"',
            // 转发，websocket，邓龙光(连公司有线局域网，端口：8099) ws://192.168.1.183:8099/subscriptions
            Natapp4WS4DevURL4DLG8099A: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4DLG8099A/"`,
            // 转发，websocket，邓龙光(连刘家敏电脑的无线网络，端口：8099) ws://192.168.137.236:8099/subscriptions
            Natapp4WS4DevURL4DLG8099B: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4DLG8099B/"`,

            // 转发，许锦滨(连公司有线局域网，端口：8099) http://192.168.1.119:8099/graphql
            Natapp4DevURL4XJB8099A: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4XJB8099A/"',
            // 转发，许锦滨(连刘家敏电脑的无线网络，端口：8099) http://192.168.137.73:8099/graphql
            Natapp4DevURL4XJB8099B: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4XJB8099B/"',
            // 转发，websocket，许锦滨(连公司有线局域网，端口：8099) ws://192.168.1.119:8099/subscriptions
            Natapp4WS4DevURL4XJB8099A: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4XJB8099A/"`,
            // 转发，websocket，许锦滨(连刘家敏电脑的无线网络，端口：8099) ws://192.168.137.73:8099/subscriptions
            Natapp4WS4DevURL4XJB8099B: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4XJB8099B/"`,

            // 转发，李东明(连刘家敏电脑的无线网络，端口：8081) http://192.168.137.182:8081/graphql
            Natapp4DevURL4LDM8081A: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4LDM8081A/"',
            // 转发，websocket，李东明(连刘家敏电脑的无线网络，端口：8081) ws://192.168.137.182:8081/subscriptions
            Natapp4WS4DevURL4LDM8081A: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4LDM8081A/"`,

            // 转发，戴海涛(连公司有线局域网，端口：8081) http://192.168.1.147:8081/graphql
            Natapp4DevURL4DHT8081A: isPro
                                    ? '"/"'
                                    : '"/Natapp4DevURL4DHT8081A/"',
            // 转发，websocket，戴海涛(连公司有线局域网，端口：8081) ws://192.168.1.147:8081/subscriptions
            Natapp4WS4DevURL4DHT8081A: isPro
                                       ? `${ str1 }"/"`
                                       : `${ str1 }"/Natapp4WS4DevURL4DHT8081A/"`,

            // 在公司开发时用，Superset，开发服务器(连公司有线局域网，端口：8081) http://192.168.1.168:8081
            devURL8081A4Superset: isPro
                                  ? '"/"'
                                  : '"/devURL8081A4Superset/"',

            // 不在公司开发时用，Superset，开发服务器(目前指向公司刘家敏的台式机9088端口，它会转发给公司的开发服务器192.168.1.168:8081) http://sn2020a.nat300.top
            Natapp4DevURL8081A4Superset: isPro
                                         ? '"/"'
                                         : '"/Natapp4DevURL8081A4Superset/"',
        };
    },
    splitChunks_obj = {
        chunks: 'all',
        ...
        cacheGroups: require( './src/js/App.js' ).isSPA_booC
                ...
                let styleType_arr = [
                    ...
                    jsType_str = 'js|ts|mjs|cjs',
                    obj = {
                        VendorsDir_CSS: {
                            test: new RegExp( `node_modules[\\\\/].*\\.css$`, 'i' ),
                            name: 'VendorsDir_CSS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `Basic_Colors_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]styles[\\\\/]${ c }[\\\\/]basic[\\\\/](Basic.${ c }|Colors.${ c })$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `BasicDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]styles[\\\\/]${ c }[\\\\/]basic[\\\\/](?!Basic.${ c }|Colors.${ c }).*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `ComponentsDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]components[\\\\/][^(/)]+[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `WebComponentsDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]webComponents[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `PublicDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]styles[\\\\/]${ c }[\\\\/]public[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `ModulesDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]styles[\\\\/]${ c }[\\\\/]modules[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `VueComponentsDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]vue[\\\\/]components[\\\\/][^(/)]+[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `VueStylesDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]vue[\\\\/]styles[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ...( arr => {
                            ...
                            arr.forEach( ( c, i, a ) => {
                                result_obj[ `PagesDir_${ c.toLocaleUpperCase() }` ] = {
                                    test: new RegExp( `src[\\\\/]styles[\\\\/]${ c }[\\\\/]pages[\\\\/].*\\.${ c }$`, 'i' ),
                        ...
                        ToolsDir_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'ToolsDir_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        WASMBasicDir: {
                            test: new RegExp( `src[\\\\/]wasm[\\\\/]basic[\\\\/]`, 'i' ),
                            name: 'WASMBasicDir',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        WASMPublicDir: {
                            test: new RegExp( `src[\\\\/]wasm[\\\\/]public[\\\\/]`, 'i' ),
                            name: 'WASMPublicDir',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        WASMModulesDir: {
                            test: new RegExp( `src[\\\\/]wasm[\\\\/]modules[\\\\/]`, 'i' ),
                            name: 'WASMModulesDir',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        WASMPagesDir: {
                            test: new RegExp( `src[\\\\/]wasm[\\\\/]pages[\\\\/]`, 'i' ),
                            name: 'WASMPagesDir',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        GQLAPIDir_JS: {
                            test: new RegExp( `src[\\\\/]graphQL[\\\\/]api[\\\\/].*\\.(graphql|gql)$`, 'i' ),
                            name: 'GQLAPIDir_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        ComponentsDir_JS: {
                            test: new RegExp( `src[\\\\/]components[\\\\/][^(/)]+[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        WebComponentsDir_JS: {
                            test: new RegExp( `src[\\\\/]webComponents[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        PublicDir_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]public[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'PublicDir_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        ModulesDir_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]modules[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'ModulesDir_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        VueRoutersDir_JS: {
                            test: new RegExp( `src[\\\\/]vue[\\\\/]routers[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        VueStoresDir_JS: {
                            test: new RegExp( `src[\\\\/]vue[\\\\/]stores[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        VueComponentsDir_JS: {
                            test: new RegExp( `src[\\\\/]vue[\\\\/]components[\\\\/][^(/)]+[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        VueModelsDir_JS: {
                            test: new RegExp( `src[\\\\/]vue[\\\\/]models[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                        ...
                        PagesDir_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]pages[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'PagesDir_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                ...
                let fun = c => {
                    ...
                    obj = {
                              ...
                              .forEach( ( c, i, a ) => {
                                  arr.forEach( ( c1, i1, a1 ) => {
                                      ...
                                      result_obj[ `Components_${ str1 }_${ c1.toLocaleUpperCase() }` ] = {
                                          test: new RegExp( `src[\\\\/]components[\\\\/]${ c }[\\\\/].*\\.${ c1 }$`, 'i' ),
                              ...
                              .forEach( ( c, i, a ) => {
                                  arr.forEach( ( c1, i1, a1 ) => {
                                      ...
                                      result_obj[ `WebComponents_${ str1 }_${ c1.toLocaleUpperCase() }` ] = {
                                          test: new RegExp( `src[\\\\/]webComponents[\\\\/]${ c }[\\\\/].*\\.${ c1 }$`, 'i' ),
                              ...
                              .forEach( ( c, i, a ) => {
                                  arr.forEach( ( c1, i1, a1 ) => {
                                      ...
                                      result_obj[ `VueComponents_${ str1 }_${ c1.toLocaleUpperCase() }` ] = {
                                          test: new RegExp( `src[\\\\/]vue[\\\\/]components[\\\\/]${ c }[\\\\/].*\\.${ c1 }$`, 'i' ),
                              ...
                              .forEach( ( c, i, a ) => {
                                  arr.forEach( ( c1, i1, a1 ) => {
                                      ...
                                      result_obj[ `VueStyles_${ str1 }_${ c1.toLocaleUpperCase() }` ] = {
                                          test: new RegExp( `src[\\\\/]vue[\\\\/]styles[\\\\/]${ c }[\\\\/].*\\.${ c1 }$`, 'i' ),
                                ...
                                fs.readdirSync( path.join( __dirname, `./src/styles/${ c1 }/pages` ) )
                                  ...
                                  .forEach( ( c, i, a ) => {
                                      ...
                                      result_obj[ `Pages_${ str1 }_${ c1.toLocaleUpperCase() }` ] = {
                                          test: new RegExp( `src[\\\\/]styles[\\\\/]${ c1 }[\\\\/]pages[\\\\/]${ c }[\\\\/].*\\.${ c1 }$`, 'i' ),
                        ...
                        VueFamily_JS: {
                            test: new RegExp( `node_modules[\\\\/](vue[\\\\/]|vue-router[\\\\/]|vuex[\\\\/]).*\\.(${ jsType_str })$`, 'i' ),
                            name: 'VueFamily_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        Echarts_JS: {
                            test: new RegExp( `node_modules[\\\\/]echarts[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'Echarts_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        jQuery_JS: {
                            test: new RegExp( `node_modules[\\\\/]jquery[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'jQuery_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        Swiper_JS: {
                            test: new RegExp( `node_modules[\\\\/]swiper[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            name: 'Swiper_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        CT_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/]CurrencyTools.esm.(${ jsType_str })$`, 'i' ),
                            name: 'CT_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        Decorator_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/]Decorator4ES6.esm.(${ jsType_str })$`, 'i' ),
                            name: 'Decorator_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        HTML2Canvas_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/]HTML2Canvas.esm.(${ jsType_str })$`, 'i' ),
                            name: 'HTML2Canvas_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        WebC_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/]WebComponents.esm.(${ jsType_str })$`, 'i' ),
                            name: 'WebC_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        Workers4MT_JS: {
                            test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/]Workers4MT.esm.(${ jsType_str })$`, 'i' ),
                            name: 'Workers4MT_JS',
                            // 数值越高越先添加加载
                            // priority: 1000,
                            enforce: true,
                        ...
                        ToolsDir_JS: ( arr => {
                            return {
                                test: new RegExp( `src[\\\\/]js[\\\\/]tools[\\\\/](?!${ arr.join( '|' ) }).*\\.(${ jsType_str })$`, 'i' ),
                                name: 'ToolsDir_JS',
                                // 数值越高越先添加加载
                                // priority: 1000,
                                enforce: true,
                            ...
                            'CurrencyTools.esm.ts',
                            'Decorator4ES6.esm.ts',
                            'HTML2Canvas.esm.ts',
                            'WebComponents.esm.ts',
                            'Workers4MT.esm.ts',

                            'CurrencyTools.esm.mjs',
                            'Decorator4ES6.esm.mjs',
                            'HTML2Canvas.esm.mjs',
                            'WebComponents.esm.mjs',
                            'Workers4MT.esm.mjs',

                            'CurrencyTools.esm.cjs',
                            'Decorator4ES6.esm.cjs',
                            'HTML2Canvas.esm.cjs',
                            'WebComponents.esm.cjs',
                            'Workers4MT.esm.cjs',
                        ] ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/graphQL/api' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `GQLAPI_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]graphQL[\\\\/]api[\\\\/]${ c }[\\\\/].*\\.(graphql|gql)$`, 'i' ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/components' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `Components_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]components[\\\\/]${ c }[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/webComponents' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `WebComponents_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]webComponents[\\\\/]${ c }[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/vue/components' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `VueComponents_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]vue[\\\\/]components[\\\\/]${ c }[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/vue/models' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `VueModels_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]vue[\\\\/]models[\\\\/]${ c }[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
                            ...
                            fs.readdirSync( path.join( __dirname, './src/js/pages' ) )
                              ...
                              .forEach( ( c, i, a ) => {
                                  ...
                                  result_obj[ `Pages_${ str1 }_JS` ] = {
                                      test: new RegExp( `src[\\\\/]js[\\\\/]pages[\\\\/]${ c }[\\\\/].*\\.(${ jsType_str })$`, 'i' ),
        ...
        return [
            {
                ...
                exclude: [
                    ...
                    path.resolve( __dirname, './src/ibms-libs/' ),
            ...
            // src/workers下的“.js”、“.ts”文件的特殊处理，使用“worker-loader”来处理，支持“.js”、“.ts”的文件，使用“Worker构造函数”
            {
                test: /\.worker\.(js|mjs|cjs)$/i,
                type: 'javascript/auto',
                parser: {
                    commonjs: true,
                    harmony: true,
                        ...
                        options: {
                            worker: {
                                ...
                                options: {
                                    type: 'classic',
                                    credentials: 'omit',
                                    name: 'my-custom-web-worker-name-js',
                                },
                            },
                            // 默认值取：output.publicPath
                            // publicPath: '../',
                            filename: `workers/[name]_[${ hashName_str }:6].js`,
                            chunkFilename: `workers/[name]_chunk_[${ hashName_str }:6].js`,
                            inline: undefined,
                            esModule: true,
                        },
                    },
                        ...
                        options: {
                            worker: {
                                ...
                                options: {
                                    type: 'classic',
                                    credentials: 'omit',
                                    name: 'my-custom-web-worker-name-ts',
                                },
                            },
                            // 默认值取：output.publicPath
                            // publicPath: '../',
            ...
            // src/workers下的“.js”、“.ts”文件的特殊处理，使用“worker-loader”来处理，支持“.js”、“.ts”的文件，使用“SharedWorker构造函数”
            {
                test: /\.sharedWorker\.(js|mjs|cjs)$/i,
                type: 'javascript/auto',
                parser: {
                    commonjs: true,
                    harmony: true,
                        ...
                        options: {
                            worker: {
                                ...
                                options: {
                                    type: 'classic',
                                    credentials: 'omit',
                                    name: 'my-custom-shared-worker-name-js',
                                },
                            },
                            // 默认值取：output.publicPath
                            // publicPath: '../',
                        ...
                        options: {
                            worker: {
                                ...
                                options: {
                                    type: 'classic',
                                    credentials: 'omit',
                                    name: 'my-custom-shared-worker-name-ts',
                                },
                            },
                            // 默认值取：output.publicPath
                            // publicPath: '../',
    ...
    cleanWebpackPluginConfig_fun = ( arg, isPro ) => ( {
        ...
        cleanOnceBeforeBuildPatterns: [
            ...
            path.join( __dirname, `./dist/${ arg }/ibms-libs/*` ),
            ...
            `!${ path.join( __dirname, `./dist/${ arg }/ibms-libs/.gitignore` ) }`,
    ...
    ForkTsCheckerNotifierWebpackPlugin_obj = {
        // 通知中显示的标题前缀。
        title: 'sn-data-center-platform-micro-front-web for TypeScript Checker Notice!!!',
        // 如果设置为true，警告将不会引起通知。
