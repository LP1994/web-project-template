...
function IsHandle19( _this, elemO ){
    let [
        opacity, // fontSize,
        width, height, paddingTop, paddingRight, paddingBottom, paddingLeft, borderTopWidth, borderRightWidth, borderBottomWidth, borderLeftWidth, marginTop, marginRight, marginBottom, marginLeft, outlineWidth,
    ] = _this.gStyle( elemO, [
        'opacity',
        // 'fontSize',
        'width',
...
class CryptoAPI {
    ...
    async getDigest2Hex4Uint8Array( dataUint8Array, algorithm = 'SHA-512' ){
        ...
        const hashArrayBuffer = await globalThis.crypto.subtle.digest( algorithm, dataUint8Array ), // convert hashArrayBuffer to byte array
            hashArray = Array.from( new Uint8Array( hashArrayBuffer ) ), // convert hashArray to hex string
    ...
    shuffleArr( arr = [] ){
        if( arr.length >= 2 ){
            ...
            for(
                ...
                index = parseInt( String( Math.random() * len ) ), temp = arr[ index ];
                   ...
                   .then( bufferSource => WebAssembly.validate( bufferSource )
                           ...
                           env: {
                               ...
                               memory: new WebAssembly.Memory( {
                                   ...
                                   initial: 100, // WebAssembly Memory的最大尺寸允许以WebAssembly pages为单位生长。
                               ...
                               table: new WebAssembly.Table( {
                                   // WebAssembly表的初始元素数。
                                   initial: 0, // 表示要存储在表中的值类型的字符串。目前，它只能有一个值“anyfunc”（函数）。
                                   element: 'anyfunc', // 允许WebAssembly Table增长的元素的最大数目。
                                   // maximum: 102400,
                               } ),
                           },
                       }, importObject ) )
        ...
        return new Proxy( {}, {
            get( target, propKey, receiver ){
                return ( {
                    ...
                    url = '', // 该参数的具体信息看CT的fetch()的第二个参数描述
                    events = {}, // 该参数的具体信息看CT的fetch()的第三个参数描述
                    options = {},
...
const toolsClass_objC = {
    CookieTool, // 服务器发送事件(Server-Sent Event)客户端类
    SSE4Client, // 用于GraphQL的各种资源上传请求的工具类
    Upload4GraphQL, // WebService4Proxy类
    WebService4Proxy, // WebSocket客户端类
    WebSocket4Client,
};
