...
export class Long {
  ...
  static fromInt( value: number, unsigned?: boolean ): Long{
    ...
    if( unsigned ){
      ...
      obj = Long.fromBits( value, ( value | 0 ) < 0
                                  ? -1
                                  : 0, true );
      if( cache ){
    ...
    obj = Long.fromBits( value, value < 0
                                ? -1
                                : 0, false );
    if( cache ){
      INT_CACHE[ value ] = obj;
  ...
  getNumBitsAbs(): number{
    if( this.isNegative() ){
      ...
      return this.eq( Long.MIN_VALUE )
             ? 64
             : this.neg().getNumBitsAbs();
    }
    const val = this.high !== 0
                ? this.high
                : this.low;
    let bit: number;
    for(
  ...
  [ Symbol.for( 'Deno.customInspect' ) ](): string{
    return `new Long("${ this.toString() }"${ this.unsigned
                                              ? ', true'
                                              : '' })`;
  }
