...
${ ( error as Error ).message }`,
  ...
  },
  {
    ...
    /**
     * 用postman测试时：<br />
     * 1、当设置为'localhost'时，用如“https://localhost:9300”能连接上（http、https、ws、wss皆是如此）。<br />
     * 2、当设置为'127.0.0.1'时，用如“https://127.0.0.1:9300”、“https://localhost:9300”能连接上（http、https、ws、wss皆是如此）。<br />
     * 3、当设置为'0.0.0.0'时，用如“https://localhost:9300”、“https://127.0.0.1:9300”、“https://192.168.10.101:9300”能连接上（http、https、ws、wss皆是如此）。<br />
     *
     * 用浏览器测试时：<br />
     * 1、当设置为'localhost'时，用如“https://localhost:9300”能连接上（http、https、ws、wss皆是如此）。<br />
     * 2、当设置为'127.0.0.1'时，用如“https://127.0.0.1:9300”、“https://localhost:9300”能连接上（http、https、ws、wss皆是如此）。<br />
     ...
     * 1、当页面地址（如“https://localhost:9300”）跟其中的websocket服务地址（如“wss://localhost:9000”）不一样时，因为端口不一致，所以也算不同的服务地址。<br />
     * 2、这时要先访问一下websocket服务地址对应的HTTP服务地址，即“https://localhost:9000”。<br />
     * 3、然后才能让页面（如“https://localhost:9300”）成功访问其中的websocket服务地址（如“wss://localhost:9000”）。<br />
     ...
     * 3、Windows系统上，浏览器不支持对0.0.0.0的直接访问，例如无法访问：https://0.0.0.0:9300。<br />
     */
    hostname: '0.0.0.0',
    onListen: (
      {
